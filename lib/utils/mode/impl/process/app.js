...
class AppUtils extends BaseAppUtils {
  fork() {
    ...
    // won't get listening event when reusePort is true, use `message` instead
    ...
    cluster.on('message', (worker, message) => {
      ...
      const appWorker = new AppWorker(worker);
      this.logger.info('[master] app_worker#%s:%s reuse-port listening on %j', appWorker.id, appWorker.workerId, message);
      this.messenger.send({
        action: 'app-start',
        data: {
          workerId: appWorker.workerId,
